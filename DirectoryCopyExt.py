# Description :- File Automation Copy all files with specified extension from first directory into the second directory
...
# Date :- 07/09/2024
######################################################################
def DirectoryTrevel(DirName1,DirName2,extension):
    ...
    if(exist == True):
        ...
        for folderName, subfolderName, filesName in os.walk(DirName1):
            ...
            for file in filesName:
                ...
                if(filepath.endswith(extension)):  # filter out files with specified extension
...
def main():
    ...
    if(len(argv) == 2): # validation of arguments 
        if((argv[1] == "-H") or (argv[1] == "-h")): # flag for display help
            print("Help : This automation script is use to copy all files with specified extension from first directory into second directory")
            exit()

        elif((argv[1] == "-U") or (argv[1] == "-u")): # flag for display usage
            print('Usage : Name_of_script.py "Path_of_First_directory" "Path_of_second_dirctory" "Extension_of_file"')
            print('Example : DirectoryCopy.py "Demo" "Anuraj" ".txt"')
            exit()
